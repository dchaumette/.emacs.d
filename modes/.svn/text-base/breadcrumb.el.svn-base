...
;;; Installation:
;;
;;  Put breadcrumb.el in the load-path and add the following to your .emacs startup file.
;;
;;  (require 'breadcrumb)
;;
;; or add the autoloads for the public commands.
;;
...
;;  (global-set-key [?\S-\040]  'bc-set)                ;; Shift-SPACE
;;  (global-set-key "\M-j"      'bc-previous)           ;; M-j
;;  (global-set-key "\S-\M-j"   'bc-next)               ;; Shift-M-j
;;  (global-set-key [M-up]      'bc-local-previous)     ;; M-up-arrow
;;  (global-set-key [M-down]    'bc-local-next)         ;; M-down-arrow.
;;  (global-set-key "\C-cj"     'bc-goto-to-current)    ;; C-c j
;;  (global-set-key "\C-x\M-j"  'bc-list)               ;; C-x M-j
;;
...
(progn
  ...
  (define-key *bc-menu-mode-map* "q"        'quit-window)
...
\\[quit-window] -- close the *Breadcrumb Bookmarks* window
"
